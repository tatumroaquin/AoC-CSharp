...
public class Part2 {
  ...
  private static long Distance((int X,int Y) p1, (int X,int Y) p2, bool[] rowMarks, bool[] colMarks) {
    ...
    for (int i = x1; i < x2; i++) {
      if (colMarks[i] == false) dist += (long) Math.Pow(10, 6) - 1;
    }

    // if between two y coords is empty space add one to dist
    for (int i = y1; i < y2; i++) {
      if (rowMarks[i] == false) dist += (long) Math.Pow(10, 6) - 1;
    }
